...
const JWTAuthAuthProvider = ({children}) => {
  ...
  jwtAxios.interceptors.response.use(
      (res) => res,
      async (err) => {
        const getRefToken = localStorage.getItem('badenRefToken')
        ...
        if (err.response.status === 401 && err.response.config.method !== "get" && getRefToken) {
          try {
            ...
            localStorage.setItem('badenToken', newToken.access)
  ...
  useEffect(() => {
    const getAuthUser = () => {
      const token = localStorage.getItem('badenToken');
      if (!token) {
        setJWTAuthData({
          user: undefined,
          isLoading: false,
  ...
  const signInUser = async ({username, password}) => {
    if (localStorage.getItem('badenRefToken')){
      localStorage.removeItem('badenRefToken')
    }

    dispatch({type: FETCH_START});
    try {
      const {data} = await jwtAxios.post(`/users/user/token/`,{username, password});
      localStorage.setItem('badenToken', data.access);
      localStorage.setItem('badenRefToken', data.refresh);
  ...
  const logout = async () => {
    localStorage.removeItem('badenToken');
    localStorage.removeItem('badenRefToken');
    // setAuthToken();
    setJWTAuthData({
      user: null,
